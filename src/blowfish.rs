...
impl Blowfish {
    pub fn new(key: &[u8], salt: &[u8]) -> Result<Self, InvalidKey> {
    ...
    fn expand_key(&mut self, key: &[u8]) {
        let mut key_pos = 0;
        for p in &mut self.p {
            *p ^= next_u32_wrap(key, &mut key_pos);
        }
        let mut lr = (0u32, 0u32);
        for i in 0..9 {
            lr = self.encrypt_p(lr.0, lr.1);
            self.p[2 * i] = lr.0;
            self.p[2 * i + 1] = lr.1;
        }
        for i in 0..4 {
    ...
    fn encrypt_p(&self, mut l: u32, mut r: u32) -> (u32, u32) {
        for i in 0..8 {
            l ^= self.p[2 * i];
    ...
    fn decrypt_p(&self, mut l: u32, mut r: u32) -> (u32, u32) {
        for i in (1..9).rev() {
            l ^= self.p[2 * i + 1];
            r ^= self.round_function(l);
            r ^= self.p[2 * i];
...
fn gen_final_blowfish_key_inplace(key: &mut [u8], salt: &[u8]) {
